...
void display(void)
{
  ...
  //drawShape();
    ...
    for (unsigned int i=0; i<BALLS->getCircles().size(); i++)
    {
...
void mouse(int mouse_button, int state, int x, int y)
{
    ...
    bool isRed;
    radius = randomNumberInRange(minRadius, maxRadius);
    ...
    isRed = false;
    BALLS->addCircle(new Circle(x, BALLS->getScreenY() - y, radius, dx, dy, r, g, b));
	}
...
{
    std::srand( time(NULL) );
	/*Create the needed amount of cricles*/
    for (int i=0; i<kNumCircles; i++)
    {
        double x, y, radius, dx, dy, r, g, b;
        radius = randomNumberInRange(minRadius, maxRadius);
		/*To provents overlapping cricles we need to keep track of them*/
        bool placeFound = false;
		
        while (i>0 && !placeFound)
        {
			/*generate random x y (inital random raius (min), current x or y - the generated radius)*/
            x = randomNumberInRange(0 + radius, BALLS->getScreenX() - radius);
            y = randomNumberInRange(0 + radius, BALLS->getScreenY() - radius);
            bool collision = false;
			/*Check for collision for the generated balls*/
            for (int j=0; j<i; j++)
            {
